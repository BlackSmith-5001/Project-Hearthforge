...
README では、このindexを v2資料の最小入口として使います。個別の v2 note は README に大量列挙しません。
...
- v2.0.0 はリリース済みです。
- v1.x profiles は引き続き有効で、deprecated ではありません。
- 最もシンプルな実用導線として、v1.x の「まずはこちら」フローから始めても構いません。
- README では、v2 index を v2資料の最小入口として使います。
